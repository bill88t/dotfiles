...
_SECTION_BG = __xonsh__.env.get(
    "XONTRIB_PROMPT_BAR_SECTION_BG", _pb_themes[_THEME]["section_bg"]
)
_SECTION_FG = __xonsh__.env.get(
    "XONTRIB_PROMPT_BAR_SECTION_FG", _pb_themes[_THEME]["section_fg"]
)
_ACCENT_FG = __xonsh__.env.get(
    "XONTRIB_PROMPT_BAR_ACCENT_FG", _pb_themes[_THEME]["accent_fg"]
)
_NOC = "{RESET}"
...
__xonsh__.env["PROMPT_FIELDS"]["hist_status"] = lambda: (
    "" if __xonsh__.history.remember_history else "hist off"
)
...
def _prompt_bar():
    try:
        ...
        if nd != d:
            printx(
                f"{{YELLOW}}The directory {d} does not exist.\nJump to parent: {nd}{{RESET}}"
            )
...
def _(cmd, rtn, **kwargs):
    if rtn not in [0, -8888] and __xonsh__.env.get(
