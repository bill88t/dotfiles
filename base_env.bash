...
build_ps1() {
    ...
    local gb=$(git_branch)
    if [[ -n $gb ]]; then
        sections+=("| \[\e[92m\]$gb\[\e[0m\]")
    fi

    local prompt_body
    prompt_body="$(echo "${sections[*]}")"

    PS1="\[\e[0m\][$prompt_body]\[\e[1;38;5;37m\]>\[\e[0m\] "
}
...
export PATH="$HOME/git/dotfiles:$HOME/.local/bin:$PATH"
